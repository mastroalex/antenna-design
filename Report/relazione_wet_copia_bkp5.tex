...
\graphicspath{{figures/}{figures/A/}{figures/B/}{figures/C/}} %Setting the graphicspath
\makeatletter
...
\tikzstyle{decision} = [diamond, minimum width=2cm, minimum height=1cm, text centered,  fill=backcolour,draw=gray]
\tikzstyle{arrow} = [thick,->,>=stealth]
\tikzstyle{process2} = [rectangle, minimum width=3.5cm, text width=3.2cm,minimum height=1cm, text centered,  fill=backcolour,draw=gray]
...
\begin{figure*}[b!]
		\begin{subfigure}{0.45\linewidth}
				\def\svgwidth{\linewidth}
			\tiny{\input{pifa_pcb_null_azymuth_comparison_tex.pdf_tex}}
			\caption{}
		\end{subfigure}
	\hfill
		\begin{subfigure}{0.45\linewidth}
				\def\svgwidth{\linewidth}
			\tiny{\input{pifa_pcb_null_elevation_comparison_tex.pdf_tex}}
...
\begin{figure*}[b!]
	\begin{subfigure}{0.45\linewidth}
		\def\svgwidth{\linewidth}
		\tiny{\input{pifa_pcb_array_azimuth_comparison_tex.pdf_tex}}
		\caption{}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.45\linewidth}
		\def\svgwidth{\linewidth}
		\tiny{\input{pifa_pcb_array_elevation_comparison_tex.pdf_tex}}
		\caption{}	\end{subfigure}
...
\begin{figure*}[b!]
	...
	\begin{tikzpicture}
		...
		\node (startp) [startstop,xshift=0.4\linewidth] {\textbf{INPUT VARIABLES}
			...
			frequency, matched input impedance, FR4 substrate, patch and ground features, PIFA condition on shorting to reach equivalence between folded structure and PIFA};		
		\node (pro1) [process, below of=startp ,yshift=-2.2cm] {
			\textbf{	THEORY}
			
			theoretical formulation to evaluate patch parameters – i.e. pre-optimized PIFA features 
		};
		\node (pro2) [process, below of=pro1,yshift=-1.8cm ] {
			\textbf{SUBSTRATE (thickness selection)}
			
			based on radiative theoretical behaviour and tool instructions
		};
		\node (pro3) [process, below of=pro2,yshift=-2.2cm] {
			\textbf{MATLAB (mesh refinement)}
			
			Study of the influence of the mesh density level parameter (maximum edge length) to the reflection coefficient and the resonant frequence. Mesh level selection so that best accuracy can be reached.
		};
		\node (pro4) [process, below of=pro3,yshift=-2.8cm] {\textbf{ MATLAB (parametrical study)}
			
			study of the influence of some of the patch parameters (i.e. length, width, feed location) on the reflection coefficient.
		};
		\node (out1p) [io, below of=pro4,yshift=-2cm] {\textbf{ OPTIMIZED PIFA}
			
			resonant at the operating frequency input and impedance matching};
		...
		{\textbf{INPUT VARIABLES}
			...
			no. of elements, mean to side lobe ratio, operating frequency};
		
		\node (out1) [io, below of=start,yshift=-2cm] {\textbf{OUTPUT VARIABLES}
			optimal inter-spacing between elements to minimize the beamwidth; feed coefficients and tapering efficiency};
		...
		circle,
		minimum size=0.6cm,
		fill=backcolour,below of=out1,yshift=-2cm] (sum) {};
		...
		\node (start_T) [startstop,below of=sum,yshift=-1cm]
		{\textbf{TOOL LIMITATIONS}};
		
		\node (pro2a) [process3, below of=start_T,yshift=- 2 cm,xshift=-2.8cm] {\textbf{PCB ANTENNA DESIGNER}
			
			PCB stack (patch, substrate, ground, feed, cylindrical shorting pins) approximating the PIFA structure};
		\node (pro2b) [process3, below of=start_T,yshift=- 2 cm,xshift=2.4cm] 
		{ \textbf{SENSOR ARRAY ANALYZER}
			
			array of PIFAs, broadside and 45° to boresight conditions};
		
		\node (pro2c) [process4, below of=pro2a,yshift=- 6 cm,xshift=2.8 cm] 
		{ \textbf{PCB ARRAY ANALYZER}
			
			array of PCBs, Broadside and 45° to boresight conditions, 2D gain patterns, electric and magnetic near fields};
		
		\node (pro2e) [process2, below of=pro2a,yshift=- 2.5cm] 
		{ \textbf{COMPARISON}
			in terms of gain/directivity patterns and error analysis};
		
		\node (pro2d) [process2, below of=pro2b,yshift=- 2.1 cm] 
		{ \textbf{COMPARISON}
			
			in terms of directivity patterns and error analysis};
		
		
		\draw [arrow] (start_T) -- (pro2a);
		\draw [arrow] (start_T) -- (pro2b);
