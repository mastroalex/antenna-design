...
\begin{table*}[bt!]
	\begin{center}
		{\fontfamily{lmss}\selectfont	\begin{tabular}{||m{4cm}|m{2.6cm}|m{2cm}|m{2cm}|m{2.3cm}|m{2.3cm}||}
				\hline
...
\begin{figure*}[b!]
		\begin{subfigure}{0.45\linewidth}
	\def\svgwidth{\linewidth}
	\tiny{\input{pcb_array_tex.pdf_tex}}
\end{subfigure}
\caption{PCB array}
\hfill
	\begin{subfigure}{0.45\linewidth}
...
\begin{figure*}[b!]
	...
	\begin{tikzpicture}
		\label{fig:flowchart}
		\node (startp) [startstop,xshift=0.4\linewidth] {\textbf{INPUT VARIABLES}
			
			\small frequency, matched input impedance, FR4 substrate, patch and ground features, PIFA condition on shorting to reach equivalence between folded structure and PIFA};		
		\node (pro1) [process, below of=startp ,yshift=-2.2cm] {
			\textbf{	THEORY}
			
			\small theoretical formulation to evaluate patch parameters – i.e. pre-optimized PIFA features 
		};
		\node (pro2) [process, below of=pro1,yshift=-1.8cm ] {
			\textbf{SUBSTRATE (thickness selection)}
			
		\small 	based on radiative theoretical behaviour and tool instructions
		};
		\node (pro3) [process, below of=pro2,yshift=-2.2cm] {
			\textbf{MATLAB (mesh refinement)}
			
		\small 	study of the influence of the mesh density level parameter (maximum edge length) to the reflection coefficient and the resonant frequence. Mesh level selection so that best accuracy can be reached.
		};
		\node (pro4) [process, below of=pro3,yshift=-2.8cm] {\textbf{ MATLAB (parametric study)}
			
		\small 	study of the influence of some of the patch parameters (i.e. length, width, feed location) on the reflection coefficient.
		};
		\node (out1p) [io, below of=pro4,yshift=-2cm] {\textbf{ OPTIMIZED PIFA}
			
		\small 	resonant at the operating frequency input and impedance matching};
		
		\node (A) [above  of=  startp,yshift=1cm]  {\textbf{\textcolor{orange_work}{Folded patch antenna design}}};
		...
		\small 	no. of elements, mean to side lobe ratio, operating frequency};
		
		\node (out1) [io, below of=start,yshift=-1.5cm] {\textbf{OUTPUT VARIABLES}
			
		\small 	optimal inter-spacing between elements to minimize the beamwidth; feed coefficients and tapering efficiency};
		\draw [arrow] (start) -- (out1);
		
		\node (B) [above  of=  start,yshift=1cm]  {\textbf{\textcolor{orange_work}{Array factor design}}};
		...
		circle,
		minimum size=0.6cm,
		fill=backcolour,below of=out1,yshift=-1.5cm] (sum) {};
		...
		{\textbf{\small TOOL LIMITATIONS}};
		
		\node (pro2a) [process3, below of=start_T,yshift=- 2 cm,xshift=-2.8cm] {\textbf{PCB ANTENNA DESIGNER}
			
		\small 	PCB stack (patch, substrate, ground, feed, cylindrical shorting pins) approximating the PIFA structure};
		\node (pro2b) [process3, below of=start_T,yshift=- 2 cm,xshift=2.4cm] 
		{ \textbf{SENSOR ARRAY ANALYZER}
			
		\small 	array of PIFAs, broadside and 45° to boresight conditions};
		
		\node (pro2c) [process4, below of=pro2a,yshift=- 6 cm,xshift=2.8 cm] 
		{ \textbf{PCB ARRAY ANALYZER}
			
		\small 	array of PCBs, Broadside and 45° to boresight conditions, 2D gain patterns, electric and magnetic near fields};
		
		\node (pro2e) [process2, below of=pro2a,yshift=- 2.5cm] 
		{ \textbf{COMPARISON}
		\small 	in terms of gain/directivity patterns and error analysis};
		
		\node (pro2d) [process2, below of=pro2b,yshift=- 2.1 cm] 
		{ \textbf{COMPARISON}
			
		\small 	in terms of directivity patterns and error analysis};
...
\end{figure*}
\begin{figure*}
	\begin{lstlisting}[language=Matlab,caption=Code,label=code]
	a=pattern();
		a=pattern()
			a=pattern()
			mypath
